...
- [class] **AUTOLOAD-SYSTEM** *ASDF/SYSTEM:SYSTEM*
    ...
    - `(`[`RECORD-SYSTEM-AUTOLOADS`][dceb] `"my-system")` will
    ...
    [`RECORD-SYSTEM-AUTOLOADS`][dceb] (e.g. because there is a
...
- [reader] **SYSTEM-RECORD-AUTOLOADS** *[AUTOLOAD-SYSTEM][cd2d] (:RECORD-AUTOLOADS = NIL)*
    ...
    autoload forms shall be written by [`RECORD-SYSTEM-AUTOLOADS`][dceb]. Conditions signalled while ASDF is compiling or loading
    the file given have a [`RECORD-SYSTEM-AUTOLOADS`][4453] restart.
...
- [function] **AUTOLOADS** *SYSTEM &KEY (PROCESS-ARGLIST T) (PROCESS-DOCSTRING T) PACKAGES*
    ...
    [`RECORD-SYSTEM-AUTOLOADS`][dceb] and [`CHECK-SYSTEM-AUTOLOADS`][4afe]
    provide [ASDF Integration][0c5c].
...
- [function] **RECORD-SYSTEM-AUTOLOADS** *SYSTEM*
    ...
    way to trigger this is to call [`RECORD-SYSTEM-AUTOLOADS`][dceb]
    ...
    [`RECORD-SYSTEM-AUTOLOADS`][dceb] may also be used as a [condition
    handler][ea6a], in which case it invokes the [`RECORD-SYSTEM-AUTOLOADS`][4453]
    restart.
...
- [function] **CHECK-SYSTEM-AUTOLOADS** *SYSTEM &KEY (ERRORP T)*
    ...
    - If there is a [`:RECORD-AUTOLOADS`][f945], then
      the file generated by [`RECORD-SYSTEM-AUTOLOADS`][dceb] is
      up-to-date.
    
    - All manual autoload declarations (i.e. [`AUTOLOAD`][7da0]s and
      [`DECLARE-VARIABLE-AUTOLOAD`][c5d0] not in the `:RECORD-AUTOLOADS` file) in
      `SYSTEM` are resolved (the corresponding function or variable is
      defined) by loading [`:AUTOLOADED-SYSTEMS`][8429].
    
    If `ERRORP`, then signal an error if the check fails or the file
    cannot be read. The [`RECORD-SYSTEM-AUTOLOADS`][4453] restart is provided.
    
    This function is called automatically by `ASDF:TEST-OP` on a
    `AUTOLOAD-SYSTEM` method if [`SYSTEM-TEST-AUTOLOADS`][3ab0].

<a id="x-28AUTOLOAD-3ARECORD-SYSTEM-AUTOLOADS-20RESTART-29"></a>

- [restart] **RECORD-SYSTEM-AUTOLOADS**

    Provided by [`CHECK-SYSTEM-AUTOLOADS`][4afe] and also when the compilation of
    the autoloads file declared in [`:RECORD-AUTOLOADS`][f945] fails. The
    function [`RECORD-SYSTEM-AUTOLOADS`][dceb] can be used as a condition handler
